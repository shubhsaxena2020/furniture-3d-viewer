...
def extract_features_opencv(image, max_features=32000):
    ...
    if gray.shape[0] < 20 or gray.shape[1] < 20 or np.std(gray) < 0.5:
        return [], np.zeros((0, 128), dtype=np.float32)
    # Adaptive threshold: low-contrast images need lower thresholds
    ...
    sift = cv2.SIFT_create(
        nfeatures=max_features, nOctaveLayers=5,
        contrastThreshold=contrast_thresh, edgeThreshold=10, sigma=1.3
    )
...
def compute_mvs_points(images, image_paths=None, max_total_points=300000):
    """
    ...
    for idx, img in enumerate(images):
        kp, desc = extract_features_opencv(img, max_features=32000)
        all_kps.append(kp)
    ...
    all_points = []
    all_colors = []
    # Match ALL image pairs for maximum coverage
    pairs_made = 0
    for i in range(n_images):
...
def _gradient_dense_cloud(images, max_points=300000):
...
def reconstruct_mesh_advanced(points, colors, target_faces=100000):
    ...
    # Subdivide to reach target face count (target 1M+)
    if len(faces) > 0 and len(faces) < target_faces and TRIMESH_AVAILABLE:
        try:
            ...
            while current < target_faces and iterations < max_iters:
                if current * 4 > 1500000:
                    print(f"    Memory guard: stopping at {current} faces (next would exceed 1.5M)")
                    break
                    break
                sub = sub.subdivide()
...
def densify_point_cloud(points, colors):
    ...
    # Cap at 300K
    if len(pts) > 300000:
        idxs = np.random.choice(len(pts), 300000, replace=False)
        pts = pts[idxs]
        cols = cols[idxs]
...
def run_photogrammetry(image_paths: List[str],
                       output_dir: str,
                       max_features: int = 32000,
                       focal_length: Optional[float] = None,
                       project_id: Optional[str] = None,
                       target_faces: int = 300000,
    ...
    mvs_points, mvs_colors = compute_mvs_points(
        images, valid_paths, max_total_points=300000
    )
    ...
    if points is None or len(points) < 20:
        ...
        points, colors_merged = _gradient_dense_cloud(images, max_points=300000)
